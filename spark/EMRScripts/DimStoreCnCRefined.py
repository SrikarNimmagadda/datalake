...
Final_Joined_DF = spark.sql("select a.storenumber, b.DealerCode as dealer_codes, "
                          + "case when a.companycd = 'SpringMobile' then '4' when a.companycd = 'Spring' then '4'  else ' ' end as companycd, "
...
#        option("header", "true").mode("overwrite").save(DimCnCRefinedOutput + "/CSV/");
