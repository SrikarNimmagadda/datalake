...
spark = SparkSession.builder.\
        appName("SalesKPIDelivery").getOrCreate()
...
for value in list1:
    ...
    fields = fieldcolumns.split(',')
    i=0
    storenum = 'storenumber'
    emp = 'sourceemployeeid'
    for field in fields:
        fieldlist.append(field)
        i=i+1
    
    if i==1:
        if fieldlist[0].find(storenum):
            fieldcolumns = fieldlist[0] + ' as STORE_NUM'
        else:
            fieldcolumns = fieldlist[0] + ' as SRC_SYS_NM'
    else:
        if fieldlist[0].find(storenum):
            fieldcolumns = fieldlist[0] + ' as STORE_NUM,' + fieldlist[1] + ' as SRC_SYS_NM'
        else:
            fieldcolumns = fieldlist[0] + ' as SRC_SYS_NM,' + fieldlist[1] + ' as STORE_NUM'
                          
    
    print(fieldcolumns)
    if filtercondition:
        sqlstring = "select " + fieldcolumns + ",'" + kpiheader + "' as kpiname, " + expression  \
                + " as kpivalue,first_value(" + firsttablename + ".report_date) as RPT_DT,first_value(" \
                + firsttablename + ".companycd) as CO_CD from " \
        ...
        option("header", "true").save(KPIOutput + '/' + kpiheader)
