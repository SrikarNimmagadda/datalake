...
from pyspark.sql.functions import lit, regexp_replace, year, from_unixtime, unix_timestamp, substring
import boto3
import csv
...
class StoreTransactionAdjustments(object):
    def __init__(self):
        ...
        self.storeTransAdjustmentsDelivery = sys.argv[4]
        self.separator = '|'
        self.replaceChar = '%'
    ...
    def loadParquet(self):
        dfNullCheck = self.sparkSession.read.format("com.databricks.spark.csv").option("header", "true").option("treatEmptyValuesAsNulls", "true").option("inferSchema", "true").load(self.storeTransAdjustmentsIn)
        if dfNullCheck.count() > 1:
            ...
            dateStrInHeader = datetime.now().strftime('%y-%b')
            ...
            dfStoreTransAdj = dfStoreTransAdj.withColumn("year", year(from_unixtime(unix_timestamp()))).withColumn("month", substring(from_unixtime(unix_timestamp()), 6, 2))
            ...
            dfStoreTransAdj.coalesce(1).select("*").write.mode("append").partitionBy('year', 'month').format('parquet').save(self.storeTransAdjustmentsOut)

        else:
            self.makeZeroByteFile(self.storeTransAdjustmentsDelivery, 'zero_byte_file.csv')
