...
ATTShippingDetailInp = sys.argv[1]
output = sys.argv[2]
FileTime = sys.argv[3]

spark = SparkSession.builder.\
        appName("ParquetATTShippingDetail").getOrCreate()
        
dfATTShippingDetail = spark.read.format("com.databricks.spark.csv").\
                   option("header", "true").\
                   option("treatEmptyValuesAsNulls", "true").\
                   option("inferSchema", "true").\
                   load(ATTShippingDetailInp)
...
dfATTShippingDetail.coalesce(1).select("*"). \
    write.parquet(output + '/' + todayyear + '/' + todaymonth + '/' + 'ParquetATTShippingDetail' + FileTime);
