...
dfCustomer = spark.sql(
    "select a.companycode from company a where a.companytype = 'Spring Mobile - AT&T'")
dfCustomer.registerTempTable("customer")

Temp_DF = spark.sql("select a.report_date,a.storeid as storenumber,"
...
# to test for KPI################3
Temp_DF.registerTempTable("test")
Temp_DF.printSchema()
Temp_DF = spark.sql(
    "select a.report_date,a.entereddate from test a where month(a.entereddate)=month(a.report_date)")
