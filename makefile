.PHONY: \
		...
		clean deepclean \
		clean-extract-metadata clean-route-raw clean-add-jobflow-steps clean-data-processing-errors \
...
DIST_DATA_PROCESSING_ERRORS = $(DIST)/$(APPNAME)-data-processing-errors.zip
...
PACK_DATA_PROCESSING_ERRORS_LOG = $(LOGS)/pack_data_processing_errors.txt
...
pack-data-processing-errors: clean-data-processing-errors prep-target
	@echo '==> Packing data_processing_errors lambda...'
	cd functions/data_processing_errors; zip -9Dr $(DIST_DATA_PROCESSING_ERRORS) * -x *.pyc tests/* tests/*/* | tee $(PACK_DATA_PROCESSING_ERRORS_LOG)
...
clean-data-processing-errors:
	@echo '==> Cleaning old data-processing-errors package...'
	rm -f $(DIST_DATA_PROCESSING_ERRORS)
